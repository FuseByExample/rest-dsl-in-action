...
+
. Change next `Direct Grant Api` by clicking on the `on/off` button. This step is required to allow REST Apiman to use the REST APi to access the Relam managed by the Oauth2 server
+
