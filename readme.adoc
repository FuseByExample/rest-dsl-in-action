...
From the service version 1.0 created, we will create a new (= clone the previous and extend the features) service and add a Basic authentication policy. The credentials ill be managed
...
+
. Encode `2.0` as Version number and click on the button `Create Version`
+
image::images/apiman-blogservice-newservice-basic1.png[]
+
. When the screen has been refreshed. Click on the `Add policy` button which is available for the policies tab. Click on it.
+
