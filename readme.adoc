...
Like we have done for the basic authentication we will also create a realm that the OAuth2 server will use to manage the credentials, the roles and the applications authorized to access
...
+
. Next add `Fuse` as name and save the result by clicking on the `save` button
+
