...
+
NOTE: For the purposes of these lab instructions, the directory of this cloned project on your local workstation will be referred to as: `$LAB_HOME`.
. Change directory into: `$LAB_HOME` and build the maven project:
+
----
...
In order to collect the data using the ElasticSearch No SQL database, an index a name of `blog` and type `article` needs to be created.
...
. In a terminal window, change directory to `$LAB_HOME`.
. The steps previously discussed in this section can be executed using the `http` command (from the {httpie} utility) or `curl`:
.. Using the `http` utility, execute the following 3 commands:
+
----
...
----
.. or using {curl} utility, execute the following 3 commands:
+
----
