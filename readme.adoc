...
. When the ApiMan gateway will receive the HTTP requests, it will log the information (Number of requests, number of responses / type) into a local ElasticsSearch database. The `Metrics` tab that
  we have / service can allow us to check these data. You can view the responses / usage using the `Show` dropdown list and filter the services for the last hour, days ...
+
image::images/apiman-blogservice-newservie-oauth2-5.png[]
+
