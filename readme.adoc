...
----
+
. When the ApiMan gateway will recieve the requests, it will insert the information into a local ElasticsSearch database. The `eview metrics` tab that
  we have / service can allow us to check the completed or failed requests. You can view the responses / usage
+
image::images/apiman-blogservice-newservice-oauth2-5.png[]
+
. Or Select `Response Type` to see the details according to the HTTP operation type
+
