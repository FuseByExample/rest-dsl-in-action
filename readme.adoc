...
Clone this project to your local workstation and change directory into the root of the project: `rest-dsl-in-action`
...
. Change to the `routing` directory of this project: `rest-dsl-in-action/routing`
...
-----
+
If prompted as per above, use username / password credentials of `admin / admin`  (as per the default in `$FUSE_INSTALL/etc/user.properties`).
...
. Now, we will install the `gpe-fuse` profile into the `demo` Fuse Demo Managed container
