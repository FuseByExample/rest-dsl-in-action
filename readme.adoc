...
The last use case that we will cover covers Oauth2 & Openid-connect authentication & authorization. Basically, ApiMan will be used as the platform issuing a request against the OAuth2 
...
to the application interested the real service. Based on the information provided within the token like the role, the OAuth2 platform will verify the request and determine if the 
application is authorized to access the service.
...
. Return to the home page of the ApiMan Management Server and click on the ``
+
