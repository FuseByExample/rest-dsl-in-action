...
To be exhaustive, we will now extend the OAuth2 Fuse Realm to specify 2 roles (writer/reader) to restrict the access to the service according to them. Typically, a writer user will be able to PUT or DELETE 
a blog article while a reader user will only be able to query the REST services.
