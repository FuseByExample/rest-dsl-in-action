...
+
. From the setings Fuse screen, select (if not yet done) the `Direct Grant Api` by clicking on the `on/off` button that we have within the `Login` tab and save the action done.
+
