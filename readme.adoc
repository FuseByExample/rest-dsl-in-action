...
. Encode the endpoint of the REST api which is authorized to access the Oauth2 Server to request a Oauth2 Token. So, fill the field `Realm` with this url `http://localhost:8080/auth/realms/fuse`
  and add to the field `Keycloak Realm Certificate` the certificate which has been generated for the `Fuse` realm. the certificate can be retrived from the Oauth2 Server as explained just after.
+
