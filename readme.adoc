...
TODO: Verify how we can change the ${container:ip} variable & Why an IP address is required
. Add config edit property "io.fabric8.elasticsearch-insight" to set this variable "network.host" to fusehost or 127.0.0.1

----
fabric:profile-edit --pid io.fabric8.elasticsearch-insight/network.host=127.0.0.1 insight-elasticsearch.node
----


## Define Elasticsearch Data Mapping

. Run these commands against the ElasticSearch node to create the Blog index and define the mapping of the fields
----
...
+
----
http PUT http://fusehost:9200/kibana-int/dashboard/fusedemo < data/elasticsearch/dashboard.json
http PUT http://fusehost:9200/kibana-int/dashboard/fusecamel < data/elasticsearch/dashboard-camel.json
----
+
...
. Create the `DEMO_DIRECTORY` ENV variable to point to the folder containing the demo project
    ...
    export DEMO_DIRECTORY=/Users/chmoulli/RedHat/GPE/GPE-Courses/fuse-rest-dsl-demo/
...
The documentation of the REST Services has been created and is defined within the services.json file of the swagger maven module under this folder +src/main/resources/services.json+.
...
To access the documentation and run the queries against the service, here are the requests to followed

. Open a terminal within the +swagger+ maven module

    cd swagger

. Launch the script +./run_jetty.sh+ to build the project locally and start a jetty instance

    ./run_jetty.sh
    
 . When the jetty instance is started, open yur browser at this address
    
    http://localhost:8000/?url=services.json

## Security governance with Apiman & Keycloak

TODO : Describe what we will do here.
...
. Start WildFly 8 using the "standalone-apiman.xml" configuration
. Point your browser at the apiman UI and login with admin/admin123!
+
[NOTE]
====
You can use this list of instructions to download and install Apiman
...
====
+
Now that our Apiman server has been launched, we will create an organization, configure the service that we would like to manage and different security policies (Basic Authentication, OAuth & Limit rates) that we will use for this demo.
+
...
. Click on the link "Create a New Organization" and encode `Fuse` as organization name within the field `Organization Nae` of the new screen displayed. 
. Next, click on the +Create Organisation+ button.

image::images/apiman-organization.png[]

. The +Fuse+ Organization screen is displayed

image::images/apiman-fuse-organisation.png[]

. Click on +Service+ Tab of the +Fuse+ organization to register the REST Service that we will manage
+
NOTE: As we will not define different SLA to manage the service, No [plan] will be defined for the demo
+
...
. Within +New Service+ screen, add +blog-service+ as Service Name and provide a +description+ to mention that this service
  is not secured "Unsecure service. No authentication will be required to access the service". When done, click on the "Create Service" button
+
image::images/apiman-fuse-organisation-addservice.png[]
+
. You will be redirected to the +blog-service+, version +1.0+ screen where we will add the URL of the REST Service exposed by the Apache Camel
  and specify that the service is [public].
+
image::images/apiman-blogservice.png[]
+
. Click on the +implementation+ link and add the Api endpoints +http://fusehost:9191/blog/article+, select +REST+ for the +Api Type+ dropdown box.
...
----
+
NOTE: Add at the end of the Api endpoint the path of one of the CRUD service
+
WARNING: We will use the verify=no option of httpie to skip the host's SSL certificate verification
+
. Check the response
+
----
...
We will define a new security rule to be able to authenticate the HTTP request calling now the service 

. Instructions to duplicate the service
...
[source]
----
http --verify=no https://localhost:8443/apiman-gateway/fuse/blog-service/2.0/search/user/cmoulliard
...
----
+
WARNING: The request fails as we haven't passed the credentials of the user to be used
. New request
+
[source]
----
http --verify=no -a charles:demo123 https://localhost:8443/apiman-gateway/fuse/blog-service/2.0/search/user/cmoulliard
----

## Use case - Oauth2 with Keycloak

Last use case that we will cover concerns Oauth2 & Openid-connect authentication

. Use Oauth2 & Openid-connect authentication
. Add Keycloak Oauth2 Plugin
...
http PUT http://fusehost:9200/blog/article/1 < data/elasticsearch/entry.json
http PUT http://fusehost:9200/blog/article/2 < data/elasticsearch/entry.json
