...
In order to use the demo, we have to realize these tasks :
...
. Run this command `./bin/fuse` to launch the JBoss Fuse Server
. Next, within the Fuse Karaf console, issue the shell command `shell:source mvn:com.redhat.gpe/fuse/1.0/script/install` 
+
NOTE: The shell script contains some Fabric shubshell commands responsible to setup the Fabric server and two containers (elasticsearch-node and demo). The first child container will be used as Elasticsearch
database server and the demo Fuse managed container to runt the Apache Camel Routes and expose the REST and File endpoints.
+
----
...
root*                 1.0        karaf   yes          fabric                           success
                                                      ...
                                                      jboss-fuse-full
  elasticsearh-node  1.0        karaf   yes          insight-elasticsearch.datastore  success
