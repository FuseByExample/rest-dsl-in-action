...
You can also run the project locally using `mvn camel:run` at the condition that the Karaf feature `insight-elasticsearch` or the profile `insight-elasticsearch.datastore` if a fabric server has been deployed into the JBoss Fuse 6.2.

. Open a Windows or Unix terminal and move to the directory `fuse-rest-dsl-demo-master/routing`
. Launch Apache Camel
